 ...
 * <P>
 * 
 * @author CS 2334. Modified by Logan Spencer
 * @version 2016-11-04
 *          <P>
 *          Project 5<BR>
 * 
...
public class StateFrame extends JFrame
{
    ...
    public class SelectionPanel extends JPanel
    {
        ...
        public SelectionPanel()
        {
            ...
            // Variable selection
            String[] sortedVariableList = dataDefinitionList
                    .getVariableIds().toArray(new String[0]);
            Arrays.sort(sortedVariableList);
            variableList = new JList<String>(sortedVariableList);
    ...
    public class CountyPanel extends JPanel
    {
        ...
        /** Color bar to indicate the 
         * relationship between colors and values. */
         ...
         * 
         */
        private CountyDefinition selectedCounty = null;
        
        /**
         * County Panel constructor
         */
        public CountyPanel()
        {
            // enable mouse events
            ...
            this.addMouseListener(new MouseAdapter()
            {
                ...
                public void mouseClicked(MouseEvent e)
                {
                    ...
                    // What county has been clicked?
                    selectedCounty = countyDefinitionList.findCounty(x, y);
                    
                    // Open a CountyFrame for the selected county 
                    if (selectedCounty != null) 
                    {
                        CountyFrame newFrame = new CountyFrame(selectedCounty);
                        newFrame.isActive();
                    }
            ...
            colorBar = new ColorBar(100, 400, getPreferredSize().width - 200,
                    55, 100,
                    0.0, 0.15, 0.0,
                    0.0, 1.0, 0.0); 
            ...
            // Build the linked list of constraints
            constraintsYear.addEnd(constraintsMonth);
            constraintsMonth.addEnd(constraintsDay);
            constraintsStations.addEnd(constraintsYear);
            
            // Initialize constraintsStation
            for (int i = 0; i < 10; ++i)
            {
        ...
        protected void paintComponent(Graphics g)
        {
            ...
            if (dataDefinition != null)
            {
                ...
                str = dataDefinition.getUnit();
                
                // draw the units string
                g2.drawString(str, XUNITS, YUNITS);
            }
        }
        ...
        public synchronized void updateData()
        {
            ...
            List<String> selectedYears = 
                    selectionPanel.yearList.getSelectedValuesList();
            ...
            // add the appropriate years to constraintsYear
            if (selectedYears.size() > 0 && 
                    !selectionPanel.yearList.getSelectedValue().equals("All"))
            {
                for (String year : selectedYears)
                {
                    constraintsYear.add(Integer.parseInt(year));
                }
            }
            else
            {
                for (int i = 1; i < selectionPanel.yearListModel.size(); ++i)
                {
                    constraintsYear.add(Integer.parseInt(
                            selectionPanel.yearListModel.get(i)));
                }
            }
            ...
            // add the appropriate months to constraintsMonth
            if (selectedMonths.length > 0 && 
                    !selectionPanel.monthList.getSelectedValue().equals("All"))
            {
                for (int month : selectedMonths)
                {
                    constraintsMonth.add(month);
                }
            }
            else
            {
            ...
            // add the appropriate days to constraintsDay
            if (selectedDays.length > 0 && 
                    !selectionPanel.dayList.getSelectedValue().equals("All"))
            {
                for (int day : selectedDays)
                {
                    constraintsDay.add(day);
                }
            }
            else
            {
            ...
            // Find what stat button is selected
            if (selectionPanel.averageButton.isSelected())
            {
                statType = StatType.AVERAGE;
            } 
            else if (selectionPanel.minButton.isSelected()) 
            {
